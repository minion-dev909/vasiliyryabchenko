...
const getContentTypeQuery = async (type) => {
  ...
  query.value.forEach((item) => {
    // Разбиваем type на части, если есть вложенность
    const typeParts = type.split('.');
    let value;

    // Если есть вложенность, получаем значение по вложенным ключам
    if (typeParts.length > 1) {
      ...
      for (const part of typeParts) {
        if (currentValue[part] !== undefined) {
          ...
          // Если какая-либо часть пути отсутствует, прекращаем поиск
          currentValue = null;
          break;
        }
      }
      ...
      // Если вложенности нет, обрабатываем как обычно
      value = item[type];
    }

    // Добавляем значение в массив, если оно уникальное и не пустое
...
const getContentRelationTypeQuery = async (type) => {
  ...
  const arrayIds = [];
  query.value.forEach((item) => {
    // Разбиваем type на части
    const typeParts = type.split('.');
    
    let targetObject;
    // Проверяем наличие вложенности
    if (typeParts.length > 1) {
      // Если есть вложенность, используем первую часть как ключ объекта, а вторую как ключ свойства
      if (item[typeParts[0]] && item[typeParts[0]][typeParts[1]]) {
        targetObject = item[typeParts[0]][typeParts[1]];
      }
    } else {
      // Если вложенности нет, просто используем type как ключ
      if (item[type]) {
        targetObject = item[type];
      }
    }

    // Проверяем наличие _id и уникальность
    if (targetObject && targetObject._id && !arrayIds.includes(targetObject._id)) {
      arrayIds.push(targetObject._id);
...
const addFilterItem = async (type, value, operator, label) => {
  const localeSufix = locale.value !== "en" ? encodeURIComponent("_" + locale.value) : "";
  const typeParts = type.split('.'); // Разделяем type по точке

  // Определяем, есть ли вложенность в type
  let query;
  if (typeParts.length > 1) {
    // Если есть вложенность, формируем строку запроса с учетом вложенности
    query = `{ "${typeParts[0]}${localeSufix}.${typeParts[1]}": "${value}" },`;
  } else {
    // Если вложенности нет, формируем строку запроса без нее
    query = `{ "${type}": "${value}" },`;
  }

  console.log(query);
  const filterExists = filters.value.some((filter) => filter.query.trim() === query.trim());
...
const isFilterActive = (id) => {
  return filters.value.some((filter) => {
    return filter.query.includes(`${id}`);
  });
};
...
//  console.log(isFilterLabelActive);
